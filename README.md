...
- 介紹如何將Posit（2022.11之前叫作RStudio）當作書寫及成果分享平台。讀者將有機會體會到將語法及分析結果同時嵌進HTML檔案中，或是轉為Word檔與人分享的愉快過程。  
...
語法檔（.R）中的"../" 表示的是「相對路徑中的上一層目錄」的意思。詳見3.1.1 (p.42-44)。若執行here::here()這一行之後所顯示的工作路徑，不是現在這個資料夾，請你（1）再次點選此章節的.Rproj檔案，或是（2） **點選設定工作路徑**：在Posit/RStudio右下角檔案總管區，切換到語法案所在資料夾；找到檔案總管區Files標籤工具列中的齒輪（More）圖示，點選"Set as Working Directory"）。
